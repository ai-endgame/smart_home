...
impl Device {
    pub fn new(name: &str, device_type: DeviceType) -> Self {
        let brightness = if device_type == DeviceType::Light { 100 } else { 0 };
        let temperature = if device_type == DeviceType::Thermostat { Some(22.0) } else { None };
...
impl Room {
    pub fn new(name: &str) -> Self {
        Room { name: name.to_string(), device_ids: Vec::new() }
    }
}

impl fmt::Display for Room {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "Room '{}' — {} device(s)", self.name, self.device_ids.len())
    }
}
...
mod tests {
    ...
    fn test_display_impls() {
        ...
        let room = Room { name: "Office".to_string(), device_ids: vec!["a".into(), "b".into()] };
