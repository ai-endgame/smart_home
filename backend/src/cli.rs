...
pub fn run_cli() {
    ...
    loop {
        ...
        match command.as_str() {
            ...
            "add-device" => {
                if parts.len() < 3 { println!("Usage: add-device <name> <type>"); continue; }
                match DeviceType::from_str_loose(parts[2]) {
                    Some(dt) => match home.add_device(parts[1], dt) {
                        Ok(id) => println!("✅ Added device '{}' (id: {})", parts[1], &id[..8]),
                        Err(e) => println!("❌ {}", e),
                    },
                    None => println!("❌ Unknown device type '{}'. Use: light, thermostat, lock, switch, sensor", parts[2]),
                }
            }

            "remove-device" => {
                if parts.len() < 2 { println!("Usage: remove-device <name>"); continue; }
            ...
            "set-state" => {
                if parts.len() < 3 { println!("Usage: set-state <device> on|off"); continue; }
                let state = match parts[2].to_lowercase().as_str() {
                    "on" => DeviceState::On, "off" => DeviceState::Off,
                    _ => { println!("❌ State must be 'on' or 'off'."); continue; }
                };
            ...
            "set-brightness" => {
                if parts.len() < 3 { println!("Usage: set-brightness <device> <0-100>"); continue; }
            ...
            "set-temp" => {
                if parts.len() < 3 { println!("Usage: set-temp <device> <temperature>"); continue; }
            ...
            "list-devices" => {
                ...
                if devices.is_empty() { println!("No devices registered."); }
                else { println!("── Devices ({}) ──", devices.len()); for d in devices { println!("  {}", d); } }
            }

            "device-info" => {
                if parts.len() < 2 { println!("Usage: device-info <name>"); continue; }
            ...
            "add-room" => {
                if parts.len() < 2 { println!("Usage: add-room <name>"); continue; }
            ...
            "assign" => {
                if parts.len() < 3 { println!("Usage: assign <device> <room>"); continue; }
            ...
            "list-rooms" => {
                ...
                if rooms.is_empty() { println!("No rooms created."); }
                else { println!("── Rooms ({}) ──", rooms.len()); for r in rooms { println!("  {}", r); } }
            }

            "room-info" => {
                if parts.len() < 2 { println!("Usage: room-info <name>"); continue; }
                let devices = home.get_room_devices(parts[1]);
                if devices.is_empty() { println!("Room '{}' has no devices (or does not exist).", parts[1]); }
                else { println!("── Room '{}' ({} devices) ──", parts[1], devices.len()); for d in devices { println!("  {}", d); } }
            }

            "add-rule" => {
                if parts.len() < 4 { println!("Usage: add-rule <name> <trigger> <action>"); continue; }
                let trigger = match parse_trigger(parts[2]) {
                    Some(t) => t,
                    None => { println!("❌ Invalid trigger format."); continue; }
                };
            ...
            "remove-rule" => {
                if parts.len() < 2 { println!("Usage: remove-rule <name>"); continue; }
            ...
            "toggle-rule" => {
                if parts.len() < 2 { println!("Usage: toggle-rule <name>"); continue; }
                match engine.toggle_rule(parts[1]) {
                    Ok(enabled) => println!("✅ Rule '{}' is now {}.", parts[1], if enabled { "enabled" } else { "disabled" }),
                    Err(e) => println!("❌ {}", e),
                }
            }
            ...
            "list-rules" => {
                ...
                if rules.is_empty() { println!("No automation rules defined."); }
                else { println!("── Rules ({}) ──", rules.len()); for r in rules { println!("  {}", r); } }
            }
            ...
            "status" => {
                ...
                println!("╚════════════════════════════════╝");
                let on_count = devices.iter().filter(|d| d.state == DeviceState::On).count();
                if !devices.is_empty() { println!("  {} device(s) ON, {} OFF", on_count, devices.len() - on_count); }
            }
...
fn parse_trigger(s: &str) -> Option<Trigger> {
    ...
    match parts[0] {
        "state" => {
            let state = match parts[2].to_lowercase().as_str() { "on" => DeviceState::On, "off" => DeviceState::Off, _ => return None };
            Some(Trigger::DeviceStateChange { device_name: parts[1].to_string(), target_state: state })
        }
        "temp-above" => Some(Trigger::TemperatureAbove { device_name: parts[1].to_string(), threshold: parts[2].parse().ok()? }),
        "temp-below" => Some(Trigger::TemperatureBelow { device_name: parts[1].to_string(), threshold: parts[2].parse().ok()? }),
        _ => None,
    }
}
...
fn parse_action(s: &str) -> Option<Action> {
    ...
    match parts[0] {
        "state" => {
            let state = match parts[2].to_lowercase().as_str() { "on" => DeviceState::On, "off" => DeviceState::Off, _ => return None };
            Some(Action::DeviceState { device_name: parts[1].to_string(), state })
        }
        "brightness" => Some(Action::Brightness { device_name: parts[1].to_string(), brightness: parts[2].parse().ok()? }),
        "temp" => Some(Action::Temperature { device_name: parts[1].to_string(), temperature: parts[2].parse().ok()? }),
        _ => None,
    }
}
